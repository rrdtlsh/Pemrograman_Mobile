...
class PokemonRepositoryImpl @Inject constructor(
    ...
    override suspend fun getPokemonDetail(name: String): Result<PokemonDetail> {
        try {
            // ... (kode untuk cek database lokal tetap sama)

            val remoteDetail = api.getPokemonDetail(name)
            // PERBAIKAN: Prioritaskan gambar official-artwork, jika tidak ada baru pakai front_default
            val imageUrl = remoteDetail.sprites.other?.officialArtwork?.frontDefault ?: remoteDetail.sprites.frontDefault ?: ""

            val detailEntity = PokemonDetailEntity(
                id = remoteDetail.id,
                name = remoteDetail.name,
                height = remoteDetail.height ?: 0, // Jika null, anggap 0
                weight = remoteDetail.weight ?: 0, // Jika null, anggap 0
                imageUrl = imageUrl,
                types = remoteDetail.types.map { it.type.name },
                // PERBAIKAN: Logika mapping stat yang lebih aman
