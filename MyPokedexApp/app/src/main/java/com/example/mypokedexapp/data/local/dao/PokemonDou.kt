...
interface PokemonDao {
    ...
    @Query("UPDATE pokemon_detail SET isFavorite = :isFavorite WHERE name = :name")
    suspend fun setFavorite(name: String, isFavorite: Boolean)

    @Query("SELECT * FROM pokemon_detail WHERE isFavorite = 1")
    fun getFavoritePokemons(): Flow<List<PokemonDetailEntity>>
}